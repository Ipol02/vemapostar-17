...
return order;}
function buildGrid(){gridEl.innerHTML="";state.tiles=[];for(var i=0;i<TILE_COUNT;i+=1){var tile=document.createElement("button");tile.type="button";tile.className="tile-efa89d42";tile.disabled=true;tile.setAttribute("aria-label","Tile "+(i+1));tile.innerHTML='<span class="tile-inner-efa89d42">'+'<span class="tile-face-efa89d42 tile-front-efa89d42"></span>'+'<span class="tile-face-efa89d42 tile-back-efa89d42"></span>'+"</span>";tile.addEventListener("click",makeTileHandler(i));gridEl.appendChild(tile);state.tiles.push(tile);}}
function makeTileHandler(index){return function(){onTilePick(index);};}
function revealTile(index,isMine,dim){var tile=state.tiles[index];if(!tile||tile.classList.contains("revealed-efa89d42")){return;}
var back=tile.querySelector(".tile-back-efa89d42");if(back){back.innerHTML=isMine?BOMB_SVG:GEM_SVG;}
tile.classList.add("revealed-efa89d42");if(isMine){tile.classList.add("boom-efa89d42");}
if(dim){tile.classList.add("dimmed-efa89d42");}
tile.disabled=true;}
function revealRemaining(dimAll){for(var i=0;i<TILE_COUNT;i+=1){if(!state.tiles[i].classList.contains("revealed-efa89d42")){revealTile(i,state.mineMap[i]===true,dimAll);}}}
function setTilesEnabled(enabled){for(var i=0;i<TILE_COUNT;i+=1){var tile=state.tiles[i];tile.disabled=!enabled||tile.classList.contains("revealed-efa89d42");}}
...
var tile=state.tiles[index];if(!tile||tile.classList.contains("revealed-efa89d42")){return;}
...
function bust(){var lost=state.bet;revealRemaining(true);setStatus("Boom! The mine took your "+formatMoney(lost)+" bet.","lose");if(panelEl){panelEl.classList.remove("jolt-efa89d42");void panelEl.offsetWidth;panelEl.classList.add("jolt-efa89d42");}
endRound();}
